...
import com.tp.correspondence.model.OutgoingTaskModel;
 ...
 * @author Shruti
 */
public class OutgoingTaskController extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        OutgoingTaskModel outgoingTaskModel = new OutgoingTaskModel();
...
//        }
        try {
            outgoingTaskModel.setConnection(DBConnection.getConnection(ctx));
        } catch (Exception e) {
            System.out.println("error in DivisionController setConnection() calling try block" + e);
        }
        try {
            //String user_name = (String) session.getAttribute("user_name");
            //int[] userdetail = outgoingTaskModel.getUserdetail(user_name);
            ...
            if(JQString != null){
                ...
                if (JQString.equals("getOfficeList")) {
                    ...
                    String offices = outgoingTaskModel.getOfficeList(organisation_name);
                    if (offices == null)
                        out.print("");
                    else
                        out.println(offices);
                    ...
                    list = outgoingTaskModel.getKeyPersonList(q);
                else if (JQString.equals("getCorrespondenceNo"))                     
                    list = outgoingTaskModel.getCorrespondenceNo(q);
                else if(JQString.equals("getCorrespondenceTask"))
                    list = outgoingTaskModel.getCorrespondenceTask(q);
            ...
            if (task != null && task.equals("PRINTRecordList")) {
                ...
                try {
                    ...
                    jrxmlFilePath = ctx.getRealPath("/report/correspondence/outgoingTaskReport.jrxml");
                    list = outgoingTaskModel.showData(search_corr_no, search_task, search_org_from, search_org_to, search_corr_date, type_of_doc, dealing_person, -1, -1, searchreply_forward, search_Other_person, search_key_person);
                    byte[] reportInbytes = outgoingTaskModel.generateRecordList(jrxmlFilePath, list);
                    response.setContentLength(reportInbytes.length);
                    servletOutputStream.write(reportInbytes, 0, reportInbytes.length);
                    servletOutputStream.flush();
                    servletOutputStream.close();
                    outgoingTaskModel.closeConnection();
                    return;
                } catch (Exception e) {
                ...
                try {
                    ...
                    jrxmlFilePath = ctx.getRealPath("/report/correspondence/outgoingTaskReport.jrxml");
                    list = outgoingTaskModel.showData(search_corr_no, search_task, search_org_from, search_org_to, search_corr_date, type_of_doc, dealing_person, -1, -1, searchreply_forward, search_Other_person, search_key_person);
                    ByteArrayOutputStream reportInbytes = outgoingTaskModel.generateExcelList(jrxmlFilePath, list);
            ...
            try {
                if (task.equals("Cancel"))
                    outgoingTaskModel.deleteRecord(Integer.parseInt(request.getParameter("correspondence_task_id")), Integer.parseInt(request.getParameter("revision")));
                ...
                //Correspondence correspond = new Correspondence();
                    ...
                    if (task.equals("Reply")) {
                        ...
                        outgoingTaskModel.insertRecord(taskBean);//home_offce_id
                    }else if (task.equals("Save") && taskBean.getCorrespondence_task_id() == 0) {
                        outgoingTaskModel.insertRecord(taskBean);//home_offce_id
                    }else if (task.equals("Save") && taskBean.getCorrespondence_task_id() > 0) {
                        //correspond = (Correspondence) co_List.get(0);
                        outgoingTaskModel.reviseRecord(taskBean );//home_offce_id
            ...
            if(from == null)
                from = "";
            noOfRowsInTable = outgoingTaskModel.getNoOfRows(search_corr_no, search_task, search_org_from, search_org_to, search_corr_date, type_of_doc, dealing_person, searchreply_forward, search_Other_person, search_key_person);                  // get the number of records (rows) in the table.
            ...
            List<TaskSchedulerBean> list1 = null;
            int size = 0;
            list1 = outgoingTaskModel.showData(search_corr_no, search_task, search_org_from, search_org_to, search_corr_date, type_of_doc, dealing_person, lowerLimit, noOfRowsToDisplay, searchreply_forward, search_Other_person, search_key_person);
            size = list1.size();            
            ...
            request.setAttribute("correcpondenceTasklist", list1);
            request.setAttribute("doclist", outgoingTaskModel.getDoclist());
            request.setAttribute("message", outgoingTaskModel.getMessage());
            request.setAttribute("msgBgColor", outgoingTaskModel.getMsgBgColor());
            request.setAttribute("IDGenerator", new UniqueIDGenerator());
            request.setAttribute("lowerLimit", lowerLimit);
            request.setAttribute("noOfRowsTraversed", noOfRowsTraversed);
            // outgoingTaskModel.closeConnection();
            request.getRequestDispatcher("/outgoingTaskView").forward(request, response);            
        } catch (Exception e) {
            System.out.println("Error in OutgoingTask Controller" + e);
            ctx.getRequestDispatcher("/outgoingTaskView").forward(request, response);
        }
