...
import com.tp.dataEntry.model.ZoneModel;
...
public class ZoneController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException
    {
       int lowerLimit, noOfRowsTraversed, noOfRowsToDisplay = 10, noOfRowsInTable;
        ...
        zoneModel.setUser(ctx.getInitParameter("db_userName"));
        zoneModel.setPassword(ctx.getInitParameter("db_userPasswrod"));
        ...
        try {
        ...
        else if(task.equals("Save all records")||task.equals("Save As New")||task.equals("Save"))
         {
            if(task.equals("Save all records"))
                zoneModel.insertRecord(request.getParameterValues("countryName"),request.getParameterValues("zone_id"),request.getParameterValues("zoneName"),request.getParameterValues("zoneDescription"));
            else if(task.equals("Save As New"))
            {
                 String zone_id[]={"1"};
                 zoneModel.insertRecord(request.getParameterValues("countryName"),zone_id,request.getParameterValues("zoneName"),request.getParameterValues("zoneDescription"));
            }else if(task.equals("Save"))
                zoneModel.updateRecord(request.getParameter("countryName"),request.getParameter("zoneId"),request.getParameter("zoneName"),request.getParameter("zoneDescription"));
         }
        ...
        else if (task.equals("generateMapReport"))
         {
            ...
            jrxmlFilePath = ctx.getRealPath("/report/ZoneReport.jrxml");
         ...
         }else if (task.equals("generateExcelReport"))
         {
            ...
            response.setHeader("Content-Disposition", "attachment; filename=ZonalCouncil_report.xls");
            ServletOutputStream servletOutputStream = response.getOutputStream();
            listAll=zoneModel.showAllData(search);
            jrxmlFilePath = ctx.getRealPath("/report/ZoneReport.jrxml");
            ByteArrayOutputStream reportInbytes = zoneModel.generateExcelReport(jrxmlFilePath,listAll);
         ...
         ArrayList<ZoneBean> list = zoneModel.getAllRecords(lowerLimit,noOfRowsToDisplay,searchZone);
