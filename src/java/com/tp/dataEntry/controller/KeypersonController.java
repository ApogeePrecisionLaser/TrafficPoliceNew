...
import com.tp.dataEntry.model.KeypersonModel;
...
public class KeypersonController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            keyModel.setDb_password(ctx.getInitParameter("db_userPasswrod"));
        ...
        try {
            ...
            try {
                ...
                if (JQstring != null) {
                    ...
                    else if (JQstring.equals("searchMobileNo")) {
                         list = keyModel.searchMobileNo(q);
                    }
            ...
            if (task1 != null && task1.equals("PRINTRecordList")) {
                String jrxmlFilePath;
                response.setContentType("application/pdf");
               int organisation_id = 0;
                if (request.getParameter("organisation") != null && !request.getParameter("organisation").isEmpty()) {
                    organisation_id = keyModel.getOrganisation_id(request.getParameter("organisation").trim());
                }
                ServletOutputStream servletOutputStream = response.getOutputStream();
                jrxmlFilePath = ctx.getRealPath("/report/KeyPerson.jrxml");
                byte[] reportInbytes = keyModel.generateKeyperSonList(jrxmlFilePath, organisation_id);
                ...
                response.addHeader("Content-Disposition", "attachment; filename=OrganizationPerson_report.xls");
                int organisation_id = 0;
                if (request.getParameter("organisation") != null && !request.getParameter("organisation").isEmpty()) {
                    organisation_id = keyModel.getOrganisation_id(request.getParameter("organisation").trim());
                }
                ServletOutputStream servletOutputStream = response.getOutputStream();
                jrxmlFilePath = ctx.getRealPath("/report/keypersonlist.jrxml");
                ByteArrayOutputStream reportInbytes = keyModel.generateKeyperSonExcelList(jrxmlFilePath, organisation_id);
            ...
            if (task.equals("Delete")) {
                ...
                //String destination = "E:/Traffic/ImageUpload/";
                String destination = keyModel.getDestination_Path("key_person");
                response.setContentType("image/jpeg");

                if (key_person_id == 0) {
                    // if city_id was not provided, that means insert new record.

                    keyModel.insertRecord(key, itr, destination);
                } else {
                    // update existing record.
                    keyModel.updateRecord(key,itr, destination);
                }
