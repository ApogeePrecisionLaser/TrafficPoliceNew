...
ShiftWorkBench-2
 */
package com.bms.shift.model;

import com.tp.general.model.GeneralModel;
...
public class ShiftLoginModel {
 ...
 JSONObject jsonObj2 = new JSONObject();
    ...
    public List<ShiftLoginBean> showDataBean(String key_person_id) {
...
//        String query = "SELECT * FROM (SELECT cordinate_id, latitude, longitude, imei_no, key_person_id "
//                + " FROM cordinate  WHERE created_date >= '" + date + "' "
//                + " AND IF('" + key_person_id + "'='', key_person_id LIKE '%%', key_person_id='" + key_person_id + "') "
//                + "  ORDER BY cordinate_id DESC) AS sc GROUP BY key_person_id ";
    ...
    public JSONObject getAreaDetails(String key_person_id) {
        JSONObject jsonObj = new JSONObject();
        String query = " select lt.location_type_name, z.zone, w.ward_name, a.area_name, cl.location, "
                ...
                + " left join zone_new as z on dlt.zone_new_id=z.zone_new_id "
                ...
                + " where kp.key_person_id='" + key_person_id + "' ";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String zone_name = rs.getString("zone");
    ...
    public static JSONObject getName(JSONObject jsonObj, String location_type, String zone, String ward, String area, String location) {
        ...
        if (location_type.equals("city_location")) {
            query = "select z.zone_new_id,z.zone,z.zone_no,w.ward_id,w.ward_name,w.ward_no,a.area_id,a.area_name,a.area_no,cl.location_no "
                    + " from city_location as cl,zone_new as z,ward as w,area as a"
                    + " where cl.area_id=a.area_id and a.ward_id=w.ward_id and w.zone_new_id=z.zone_new_id and location='" + location + "' ";
        } else if (location_type.equals("area")) {
            query = "select z.zone_new_id,z.zone,z.zone_no,w.ward_id,w.ward_name,w.ward_no,a.area_id,a.area_no from zone_new as z,ward as w,area as a"
                    + " where a.ward_id=w.ward_id and w.zone_new_id=z.zone_new_id and area_name='" + area + "' ";
        } else if (location_type.equals("ward")) {
            query = "select z.zone_new_id,z.zone ,z.zone_no,w.ward_id,w.ward_no from zone_new as z,ward as w "
                    + " where w.zone_new_id=z.zone_new_id and ward_name='" + ward + "' ";
        } else if (location_type.equals("zone")) {
            query = "Select z.zone_new_id,zone_no from zone_new "
                    + " where zone='" + zone + "' ";
        }
        try {
            ResultSet rs = connection.prepareStatement(query).executeQuery();
            while (rs.next()) {
                if (location_type.equals("city_location")) {
                    jsonObj.put("zone_id", rs.getInt("zone_new_id"));
                    jsonObj.put("zone_name", rs.getString("zone"));
    ...
    public JSONArray getBeneficiaryDetails(String zone, String ward, String area) {
        ...
        String query = "   select beneficiary_id,kp.key_person_id,kp.salutation,kp.key_person_name,kp.age,kp.email_id1,kp.father_name, "
                ...
                + "  from  beneficiary as b,city_location as cl,area as a,ward as w,zone_new as z,key_person as kp,type_of_occupation as too "
                ...
                + "  a.ward_id=w.ward_id and w.zone_new_id=z.zone_new_id and b.key_person_id=kp.key_person_id "
                + "  and zone='" + zone + "' and ward_name='" + ward + "' and area_name='" + area + "' ";
    ...
    public JSONArray getCityDetails(String zone, String ward, String area) {
        JSONArray jsonArray = new JSONArray();
        String query = "  select cl.city_location_id,cl.location,location_no,cl.latitude,cl.longitude "
                + " from city_location as cl,area as a,ward as w,zone_new as z "
                + " where  cl.area_id=a.area_id and  a.ward_id=w.ward_id and w.zone_new_id=z.zone_new_id "
                + " and zone='" + zone + "' and ward_name='" + ward + "' and area_name='" + area + "' ";
    ...
    public int insertCordinate(String latitude, String longitude, String key_person_id, String imei_no, String mobile_no) {
        int rowsAffected = 0;
        String query = "insert into cordinate(latitude,longitude,key_person_id,imei_no,mobile_no) values(?,?,?,?,?)";
        try {
            PreparedStatement ps = (PreparedStatement) connection.prepareStatement(query);
            double latitude1 = Double.parseDouble(latitude);
            if (latitude1 == 0) {
    ...
    public String getskpmId(String key_person_id) {
        int skpm_id = 0;
        String attendence = "";
        ...
        try {
            ...
            String query = "select a.shift_key_person_map_id,attendence from attendence as a,shift_key_person_map as skpm "
                    + " where a.shift_key_person_map_id=skpm.shift_key_person_map_id and skpm.key_person_id='" + key_person_id + "' and skpm.date='" + current_date + "' ";
            ResultSet rset = connection.prepareStatement(query).executeQuery();
            while (rset.next()) {
    ...
    public int getKeyPersonId(String mobile_no) {
     ...
     public JSONArray getOffenderId(String mobile_no) {
         JSONArray jsonArray = new JSONArray();
        // jsonObj2.clear();
        int rowsAffected = 0;
    ...
    public JSONObject getKeyPersonDetail(String mobile_no) {
        JSONObject jsonObj = new JSONObject();
        String query = "select key_person_id,key_person_name,emp_code,mobile_no1,address_line1,d.designation "
                + " FROM key_person as kp,designation as d "
